...
class ProductTemplate(models.Model):
    ...
    # document_po_ids = fields.Many2many('documents.document','document_product_tmpl_po_rel','product_tmpl_id','document_id', string='PO Documents',) 
...
#depr
    ...
    def _onchange_attachment_po_ids(self):
        ...
        for new_attach in self.attachment_po_ids:
            if new_attach.id.origin not in self._origin.attachment_po_ids.ids:
                _logger.debug("added an attachment")
    ...
    def _add_po_tag(self, document):
        po_tag = self.env.user.company_id.product_document_tag_po
        if po_tag not in document.tag_ids:
            _logger.info("adding tag")
            document.write({'tag_ids':[(4,po_tag.id,_)]})

    def _remove_po_tag(self, document):
        po_tag = self.env.user.company_id.product_document_tag_po
        if po_tag in document.tag_ids:
            _logger.info("removing tag")
