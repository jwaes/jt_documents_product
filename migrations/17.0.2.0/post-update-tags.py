...
def migrate(cr, version):
    ...
    attach_ids = util.env(cr)['documents.document'].search([['tag_ids', 'in', po_tag.id]]).attachment_id.ids
    ...
    attach_ids = util.env(cr)['documents.document'].search([['tag_ids', 'in', so_tag.id]]).attachment_id.ids
    ids = util.env(cr)['product.document'].search([['ir_attachment_id', 'in', attach_ids ]]).ids
